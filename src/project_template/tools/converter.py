#written by Gilzamir F. Gomes (gilzamir_gomes@uvanet.br)
import sys
...
def write_xyz(file, varname, sufix, list, isToPrintCounts):
	...
	if isToPrintCounts:
		file.write("const GLuint %s_countVertices=%s;\n\n"%(varname, n))
...
def write_index(file, varname, sufix, list, isToPrintCounts):
	...
	n = len(list)
	if isToPrintCounts:
		file.write("const GLuint %s_countPrimitives=%s;\n\n"%(varname, n))
	...
	if isToPrintCounts:
		file.write("const GLuint %s_primitiveSize=%s;\n\n"%(varname,verticesInFace))


def generate(file, varname):
	write_xyz(file, varname, "vertexList", vertices, True)
	write_xyz(file, varname, "normalList", normals, False)
	write_index(file, varname, "indexList", face_index, True)
	write_index(file, varname, "normalIndexList", normal_index, False)
...
def main():
	...
	#outhppfile.write("extern const GLfloat %s_vertexList[];\n"%(varbasename))
	#outhppfile.write("extern const GLfloat %s_normalList[];\n"%(varbasename))
	#outhppfile.write("extern const GLuint %s_indexList[];\n"%(varbasename))
	#outhppfile.write("extern const GLuint %s_normalIndexList[];\n"%(varbasename))
