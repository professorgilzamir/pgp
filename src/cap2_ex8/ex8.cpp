...
GraphicObject* newCube(Shader *shader, GLfloat size) {
	...
	colors[0] = 1.0f; colors[1] = 1.0f; colors[2] = 0.0f; colors[3] = 1.0;

	vertices[3] = -p; vertices[4] = p; vertices[5] = p;
	colors[4] = 1.0f; colors[5] = 1.0f; colors[6] = 0.0f; colors[7] = 1.0;
		
	vertices[6] = -p; vertices[7] = -p; vertices[8] = p;
	colors[8] = 1.0f; colors[9] = 1.0f; colors[10] = 0.0f; colors[11] = 1.0;
	
	vertices[9] = p; vertices[10] = -p; vertices[11] = p;
	colors[12] = 1.0f; colors[13] = 1.0f; colors[14] = 0.0f; colors[15] = 1.0;
...
int inicializar(void)
{
	...
	try{
		...
		GLfloat translate = 0.1f;

		projection = glm::perspective(glm::radians(45.0f), 4.0f / 3.0f, 0.001f, 100.f);
		camera = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -translate));
		camera = glm::rotate(camera, glm::radians(10.0f), glm::vec3(-1.0f, 0.0f, 0.0f));
		camera = glm::rotate(camera, glm::radians(10.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		glm::mat4 model = glm::scale(glm::mat4(1.0f), glm::vec3(0.2f));

		cubo = new GraphicObject*[NCUBES];
		for (int i = 0; i < NCUBES; i++) {
			cubo[i] = newCube(shader, 0.2f);
			cubo[i]->primitive = GL_QUADS;
			...
			cubo[i]->transform = model;
			cubo[i]->initShader();
			scene.objects.push_back( cubo[i] );
		}
		shader->init();
		shader->bindUDataMatrix4fv("projection",  1, false, glm::value_ptr(projection));
		shader->bindUDataMatrix4fv("camera", 1, false, glm::value_ptr(camera));
		shader->bindUDataMatrix4fv("world", 1, false, glm::value_ptr(world));
